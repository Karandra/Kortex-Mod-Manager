...
KPackageCreatorPageBase::KPackageCreatorPageBase(KPackageCreatorWorkspace* mainWorkspace, KPackageCreatorController* controller)
	:KWorkspace(mainWorkspace->GetMainWindow(), mainWorkspace->m_PagesBook), m_MainWorkspace(mainWorkspace), m_Controller(controller)
{
	m_MainSizer = new wxBoxSizer(wxVERTICAL);
}
KPackageCreatorPageBase::~KPackageCreatorPageBase()
{
}
...
wxString KPackageCreatorPageBase::OnGetWindowTitle() const
{
	wxString name = m_Controller ? m_Controller->GetProjectFileName() : KPackageCreatorController::GetNewProjectName();
	bool hasChanges = m_Controller ? m_Controller->HasUnsavedChanges() : false;
	return wxString::Format("%s – %s%s", KWorkspace::OnGetWindowTitle(), name, hasChanges ? "*" : "");
}

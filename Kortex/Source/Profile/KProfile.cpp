...
void KProfile::Load()
{
	KxFileStream stream(GetOrderFile(), KxFS_ACCESS_READ, KxFS_DISP_OPEN_EXISTING, KxFS_SHARE_READ);
...
void KProfile::Save()
{
	...
	KxFileStream stream(GetOrderFile(), KxFS_ACCESS_WRITE, KxFS_DISP_CREATE_ALWAYS, KxFS_SHARE_READ);
	xml.Save(stream);
}
